...
## Объём работ
- **В рамках MVP**: жизненный цикл listener-бота, загрузка ролей из `roles/agents.yaml`, файловое хранилище диалогов, последовательное выполнение ролей с ACP- и SDK-коннекторами, рассылка ответов через бот-актеров, базовые команды (`/start`, `/help`, `/reset`, `/status`, `/agents`, `/next`, `/about`, `/metrics`), минимальный веб-интерфейс (`web_server.py`) для редактирования ролей и токенов.
...
- CLI-агенты (OpenCode, Qwen, Gemini и др.) устанавливаются и аутентифицируются пользователем вне NeuroCrew; SDK-агенты (OpenAI, Anthropic) используют API-ключи из пользовательского окружения (`~/.bashrc`, `~/.zshrc`), не из `.env`.
...
3. **Выполнение коннекторов**  
   ...
   - SDK-коннекторы (OpenAI/Anthropic) вызывают официальные клиенты, считывая ключи из окружения.  
   - При сбое делаем до двух повторов, затем возвращаем ошибку.
4. **Хранение диалогов**  
   - История лежит в `data/conversations/chat_{id}.json` с метаданными; длина ограничена `MAX_CONVERSATION_LENGTH`.
5. **Выход в Telegram**  
...
6. **Команды и обслуживание**  
   ...
   - `web_server.py` позволяет обновлять `roles/agents.yaml` и токены через форму с базовой авторизацией.
...
- При необходимости запускается `python web_server.py` (порт 8080) с Basic Auth.
