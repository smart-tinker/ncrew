...
- 🔌 **Единый подход через CLI** — каждый провайдер управляет собственным контекстом и авторизацией; NeuroCrew просто стримит запросы/ответы.
...
- ⚡ **User-controlled auth** — ответственность за авторизацию CLI лежит на операторе.
- 🚀 **Protocol-agnostic** — добавление нового провайдера = ещё один CLI-коннектор.
