...
- Папка `connectors/` содержит ACP-модули (`opencode_acp_connector.py`, `qwen_acp_connector.py`, `gemini_acp_connector.py`) и SDK-модули (`openai_sdk_connector.py`, `anthropic_sdk_connector.py`) поверх `base.py`/`base_sdk_connector.py`.
...
- `pytest` — полный тест-ран; для точечных проверок есть `tests/test_opencode_acp.py`, `tests/test_openai_sdk.py`, `tests/test_web_server.py` и т.д.
...
- Асинхронные функции не блокируют event loop: ввод/вывод и запуск CLI выносим в обёртки `BaseConnector`/`BaseSDKConnector`.
...
- CLI-агенты (OpenCode, Qwen, Gemini) аутентифицируются локально; SDK-агенты используют `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` из пользовательского окружения, эти переменные не прописываем в `.env`.
