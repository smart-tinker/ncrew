...
- **Веб-интерфейс конфигурации**: минимальная панель (`app/interfaces/web_server.py` + `templates/index.html`) для редактирования ролей и токенов.
...
Пользователь → Telegram-группа → Listener Bot → NeuroCrew Core → Цепочка ролей
                                                   ↓
                     ACP-коннекторы (OpenCode/Qwen/Gemini) и Headless CLI (Codex/Claude)
                                                   ↓
...
- **`main.py`** — точка входа, настраивает логирование, запускает Telegram-бот и Flask web-сервер.
- **`app/core/engine.py`** — **NeuroCrewLab**, ядро системы: оркестрация ролей, управление коннекторами, автономные циклы.
- **`app/interfaces/telegram_bot.py`** — обрабатывает входящие сообщения, команды и рассылает ответы через бот-актёров.
- **`app/interfaces/web_server.py`** — Flask-приложение с Basic Auth для правки ролей и токенов через браузер.
- **`app/connectors/`**  
...
- **`app/storage/file_storage.py`** — асинхронное сохранение истории, резервное копирование повреждённых файлов.
- **`app/config.py` + `roles/agents.yaml`** — описание ролей, загрузка токенов `{TELEGRAM_BOT_NAME}_TOKEN`, валидация настроек.
- **`templates/index.html`** — форма редактирования ролей (имя, prompt-файл, `agent_type`, `cli_command`, токены).
- **`app/utils/`** — логгеры, форматтеры, утилиты безопасности.

### Поток обработки

1. Listener Bot принимает сообщение, проверяет чат и безопасность (`app.utils.security`).
...
3. `NeuroCrewLab.handle_message` запускает цикл ролей (round-robin). Для каждой роли:
   ...
   - Обеспечивает активность коннектора (ACP-процесс или CLI-сессию).
...
5. **Веб-панель ролей**
   ...
   - Для отдельной от бота работы можно вручную запустить `python app/interfaces/web_server.py` (Basic Auth остаётся той же).
...
├── templates/
│   └── index.html                  # Форма редактирования ролей
├── roles/
│   ├── agents.yaml                 # Конфигурация ролей
│   └── prompts/                    # Промпты ролей
├── data/{conversations,logs}/       # Рабочие данные (не коммитим)
├── tests/                          # Тесты
...
- Локальная диагностика:  
  - `LOG_LEVEL=DEBUG python main.py` — подробные логи ACP/CLI.  
...
3. **Таймауты агентов** — увеличьте `AGENT_TIMEOUT` или `GEMINI_MAX_TIMEOUTS`, если CLI отвечает дольше.  
...
- **Планы:** Добавление новых провайдеров, улучшение наблюдаемости, расширение сценариев безопасности.

## 📚 Подробная документация

- **[📖 Архитектура](docs/ARCHITECTURE.md)** — детальное описание компонентов и data flow
- **[🔌 Коннекторы](docs/CONNECTORS.md)** — руководство по AI-интеграциям
- **[🤖 Telegram-бот](docs/TELEGRAM_BOT.md)** — команды и работа с Telegram
- **[🌐 Веб-сервер](docs/WEB_SERVER.md)** — панель управления ролями
- **[💾 Файловое хранилище](docs/FILE_STORAGE.md)** — работа с историей диалогов
- **[⚙️ Установка и развертывание](docs/SETUP_AND_DEPLOYMENT.md)** — полное руководство по настройке
...
- добавляют новые коннекторы (ACP или headless CLI),
